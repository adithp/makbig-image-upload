...
const pb = new PocketBase('http://127.0.0.1:8090'); // Default PocketBase URL
...
export const authHelpers = {
  ...
  // Register user
  async register(name: string, email: string, password: string, domain: string) {
    try {
      const data = {
        name,
        email,
...
export const fileHelpers = {
  // Upload image
  async uploadImage(file: File, studentId: string, week: number, imageNumber: number) {
    try {
