...
# This script helps you set up the database collections for Makbig Academy
...
Write-Host "- Create: @request.auth.role = 'student' && @request.auth.id = student_id" -ForegroundColor Gray
Write-Host "- Update: @request.auth.role = 'admin' || (@request.auth.id = student_id && admin_reply = '')" -ForegroundColor Gray
Write-Host "- Delete: @request.auth.role = 'admin'" -ForegroundColor Gray
Write-Host ""
